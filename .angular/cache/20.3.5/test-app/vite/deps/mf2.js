    ...
    var Parser = (
      ...
      (function() {
        ...
        Parser2.prototype.onclosetag = function(start, endIndex) {
          ...
          if (!this.isVoidElement(name2)) {
            var pos2 = this.stack.lastIndexOf(name2);
            if (pos2 !== -1) {
        ...
        Parser2.prototype.getInstructionName = function(value2) {
          var index = value2.search(reNameEnd);
          var name2 = index < 0 ? value2 : value2.substr(0, index);
          if (this.lowerCaseTagNames) {
            name2 = name2.toLowerCase();
          }
          return name2;
        ...
        Parser2.prototype.oncdata = function(start, endIndex, offset2) {
          ...
          this.endIndex = endIndex;
          var value2 = this.getSlice(start, endIndex - offset2);
    ...
    (function(root, factory) {
      ...
      return function(input) {
        ...
        function collectCharacters(regEx) {
          var chars, match = regEx.exec(input.substring(pos2));
          if (match) {
            chars = match[0];
            pos2 += chars.length;
            return chars;
        ...
        var inputLength = input.length, regexLeadingSpaces = /^[ \t\n\r\u000c]+/, regexLeadingCommasOrSpaces = /^[, \t\n\r\u000c]+/, regexLeadingNotSpaces = /^[^ \t\n\r\u000c]+/, regexTrailingCommas = /[,]+$/, regexNonNegativeInteger = /^\d+$/, regexFloatingPoint = /^-?(?:[0-9]+|[0-9]*\.[0-9]+)(?:[eE][+-]?[0-9]+)?$/, url, descriptors, currentDescriptor, state, c, pos2 = 0, candidates = [];
        while (true) {
        ...
        function parseDescriptors() {
          var pError = false, w, d, h, i, candidate = {}, desc, lastChar, value2, intVal, floatVal;
    ...
    var Node = class {
      ...
      positionBy(opts = {}) {
        ...
        if (opts.index) {
          ...
          if (index !== -1) pos2 = this.positionInside(index);
        }
        return pos2;
      }
      ...
      error(message, line, column, opts = {}) {
        ...
        if (line && typeof line === "object") {
          ...
          if (typeof start.offset === "number") {
            ...
            let pos2 = this.fromOffset(offset2);
            line = pos2.line;
            column = pos2.col;
          } else {
            line = start.line;
          ...
          if (typeof end.offset === "number") {
            ...
            let pos2 = this.fromOffset(endOffset);
            endLine = pos2.line;
            endColumn = pos2.col;
          } else {
            endLine = end.line;
        ...
        switch (code) {
          ...
          case OPEN_PARENTHESES: {
            ...
            if (prev === "url" && n !== SINGLE_QUOTE && n !== DOUBLE_QUOTE && n !== SPACE && n !== NEWLINE && n !== TAB && n !== FEED && n !== CR) {
              ...
              content = css.slice(pos2, next + 1);
              if (next === -1 || RE_BAD_BRACKET.test(content)) {
                currentToken = ["(", "(", pos2];
              } else {
                currentToken = ["brackets", content, pos2, next];
                pos2 = next;
              }
            }
            break;
          ...
          default: {
            if (code === SLASH && css.charCodeAt(pos2 + 1) === ASTERISK) {
...
`);
      ...
      const parser = new htmlparser.Parser({
        onopentag: function(name2, attribs) {
          if (options2.onOpenTag) {
            options2.onOpenTag(name2, attribs);
          }
          ...
          if (skip) {
            if (options2.disallowedTagsMode === "discard" || options2.disallowedTagsMode === "completelyDiscard") {
              if (frame.innerText && !hasText) {
                ...
                if (options2.textFilter) {
                  result += options2.textFilter(escaped, name2);
                } else {
                  result += escaped;
                }
                addedText = true;
          ...
          const isBeingEscaped = skip && (options2.disallowedTagsMode === "escape" || options2.disallowedTagsMode === "recursiveEscape");
          const shouldPreserveEscapedAttributes = isBeingEscaped && options2.preserveEscapedAttributes;
          if (shouldPreserveEscapedAttributes) {
            each(attribs, function(value2, a) {
              result += " " + a + '="' + escapeHtml(value2 || "", true) + '"';
            });
          } else if (!allowedAttributesMap || has(allowedAttributesMap, name2) || allowedAttributesMap["*"]) {
            each(attribs, function(value2, a) {
              if (!VALID_HTML_ATTRIBUTE_NAME.test(a)) {
                delete frame.attribs[a];
                return;
              }
              if (value2 === "" && !options2.allowedEmptyAttributes.includes(a) && (options2.nonBooleanAttributes.includes(a) || options2.nonBooleanAttributes.includes("*"))) {
                delete frame.attribs[a];
                return;
              }
              ...
              if (!allowedAttributesMap || has(allowedAttributesMap, name2) && allowedAttributesMap[name2].indexOf(a) !== -1 || allowedAttributesMap["*"] && allowedAttributesMap["*"].indexOf(a) !== -1 || has(allowedAttributesGlobMap, name2) && allowedAttributesGlobMap[name2].test(a) || allowedAttributesGlobMap["*"] && allowedAttributesGlobMap["*"].test(a)) {
                ...
                for (const o of allowedAttributesMap[name2]) {
                  if (isPlainObject(o) && o.name && o.name === a) {
                    ...
                    } else if (o.values.indexOf(value2) >= 0) {
                      newValue = value2;
                    }
                    value2 = newValue;
                  }
              ...
              if (passedAllowedAttributesMapCheck) {
                ...
                if (a === "srcset") {
                  try {
                    ...
                    if (!parsed.length) {
                      ...
                      value2 = stringifySrcset(filter(parsed, function(v) {
                        return !v.evil;
                      }));
                      frame.attribs[a] = value2;
                    }
                  } catch (e) {
                    delete frame.attribs[a];
                    return;
                ...
                if (a === "class") {
                  ...
                  if (allowedSpecificClasses && allowedWildcardClasses) {
                    value2 = filterClasses(value2, deepmerge(allowedSpecificClasses, allowedWildcardClasses), allowedClassesGlobs);
                  } else {
                ...
                if (a === "style") {
                  if (options2.parseStyleAttributes) {
                    try {
                      const abstractSyntaxTree = postcssParse(name2 + " {" + value2 + "}", { map: false });
        ...
        onclosetag: function(name2, isImplied) {
          if (options2.onCloseTag) {
            options2.onCloseTag(name2, isImplied);
          }
          if (skipText) {
            skipTextDepth--;
            if (!skipTextDepth) {
          ...
          if (options2.exclusiveFilter) {
            const filterResult = options2.exclusiveFilter(frame);
      ...
      function naughtyHref(name2, href) {
        ...
        if (has(options2.allowedSchemesByTag, name2)) {
...
// ../../node_modules/messageformat/lib/cst/names.js
var bidiChars = /^[\u061c\u200e\u200f\u2066-\u2069]+/;
var nameChars = /^[-.+0-9A-Z_a-z\u{a1}-\u{61b}\u{61d}-\u{167f}\u{1681}-\u{1fff}\u{200b}-\u{200d}\u{2010}-\u{2027}\u{2030}-\u{205e}\u{2060}-\u{2065}\u{206a}-\u{2fff}\u{3001}-\u{d7ff}\u{e000}-\u{fdcf}\u{fdf0}-\u{fffd}\u{10000}-\u{1fffd}\u{20000}-\u{2fffd}\u{30000}-\u{3fffd}\u{40000}-\u{4fffd}\u{50000}-\u{5fffd}\u{60000}-\u{6fffd}\u{70000}-\u{7fffd}\u{80000}-\u{8fffd}\u{90000}-\u{9fffd}\u{a0000}-\u{afffd}\u{b0000}-\u{bfffd}\u{c0000}-\u{cfffd}\u{d0000}-\u{dfffd}\u{e0000}-\u{efffd}\u{f0000}-\u{ffffd}\u{100000}-\u{10fffd}]+/u;
var notNameStart = /^[-.0-9]/;
function parseNameValue(source2, start) {
  let pos2 = start;
  const startBidi = source2.slice(pos2).match(bidiChars);
  if (startBidi)
    pos2 += startBidi[0].length;
  const match = source2.slice(pos2).match(nameChars);
  if (!match)
    return null;
  const name2 = match[0];
  if (notNameStart.test(name2))
    return null;
  pos2 += name2.length;
  const endBidi = source2.slice(pos2).match(bidiChars);
  if (endBidi)
    pos2 += endBidi[0].length;
  return { value: name2.normalize(), end: pos2 };
}
var parseUnquotedLiteralValue = (source2, start) => source2.slice(start).match(nameChars)?.[0] ?? "";

// ../../node_modules/messageformat/lib/data-model/from-cst.js
var cstKey = Symbol.for("CST");

// ../../node_modules/messageformat/lib/errors.js
...
var MessageSyntaxError = class extends MessageError {
  start;
  end;
  /** @private */
...
var MessageDataModelError = class extends MessageSyntaxError {
  /** @private */
...
var MessageResolutionError = class extends MessageError {
  source;
  cause;
  constructor(type, message, source2, cause) {
    super(type, message);
    this.source = source2;
    if (cause !== void 0)
      this.cause = cause;
...
var MessageFunctionError = class extends MessageError {
  source;
  cause;
  constructor(type, message) {
    super(type, message);
...
// ../../node_modules/messageformat/lib/data-model/parse.js
var bidiChars2 = new Set("؜‎‏⁦⁧⁨⁩");
var whitespaceChars = new Set("	\n\r 　");
var pos;
var source;
var MissingSyntax = (pos2, expected) => new MessageSyntaxError("missing-syntax", pos2, pos2 + expected.length, expected);
var SyntaxError = (...args) => new MessageSyntaxError(...args);
...
function selectMessage(declarations2) {
  pos += 6;
  ws(true);
  const selectors = [];
  while (source[pos] === "$") {
  ...
  return { type: "select", declarations: declarations2, selectors, variants };
}
function variant() {
  const keys = [];
  while (pos < source.length) {
...
function declarations() {
  ...
  loop: while (source[pos] === ".") {
    ...
    switch (keyword) {
      case ".input":
        declarations2.push(inputDeclaration());
        break;
      case ".local":
        declarations2.push(localDeclaration());
        break;
      case ".match":
        break loop;
      default:
...
function inputDeclaration() {
  ...
  const value2 = expression(false);
  if (value2.type === "expression" && value2.arg?.type === "variable") {
    return { type: "input", name: value2.arg.name, value: value2 };
  }
...
function localDeclaration() {
  ...
  expect("{", false);
  const value2 = expression(false);
  return { type: "local", name: name_, value: value2 };
}
function expression(allowMarkup) {
  ...
  expect("}", true);
  if (functionRef) {
    const exp = arg ? { type: "expression", arg, functionRef } : { type: "expression", functionRef };
    if (attributes_)
  ...
  return attributes_ ? { type: "expression", arg, attributes: attributes_ } : { type: "expression", arg };
}
function options() {
  ws("/}");
  const options2 = {};
  ...
  while (pos < source.length) {
    ...
    if (Object.hasOwn(options2, name_)) {
      throw SyntaxError("duplicate-option-name", start, pos);
    }
    ws();
    expect("=", true);
    ws();
    options2[name_] = value(true);
    isEmpty = false;
    ws("/}");
  }
  return isEmpty ? null : options2;
}
function attributes() {
  const attributes2 = {};
  let isEmpty = true;
  while (source[pos] === "@") {
    ...
    pos += 1;
    const name_ = identifier();
    if (Object.hasOwn(attributes2, name_)) {
    ...
    if (source[pos] === "=") {
      pos += 1;
      ws();
      attributes2[name_] = literal(true);
      ws("/}");
    } else {
      attributes2[name_] = true;
    }
    isEmpty = false;
  }
  return isEmpty ? null : attributes2;
}
function text() {
  let value2 = "";
  let i = pos;
  loop: for (; i < source.length; ++i) {
    switch (source[i]) {
      case "\\": {
        ...
        value2 += source.substring(pos, i) + esc;
        i += 1;
        pos = i + 1;
        break;
      }
...
function literal(required) {
  ...
  const value2 = parseUnquotedLiteralValue(source, pos);
  if (!value2) {
    if (required)
      throw SyntaxError("empty-token", pos);
    else
...
function name() {
  const name2 = parseNameValue(source, pos);
  if (!name2)
    throw SyntaxError("empty-token", pos);
  pos = name2.end;
  return name2.value;
...
function ws(req = false) {
  ...
  while (bidiChars2.has(next) || whitespaceChars.has(next))
    next = source[++pos];
...
// ../../node_modules/messageformat/lib/data-model/visit.js
function visit(msg, visitors) {
  const { node, pattern: pattern2 } = visitors;
  const { functionRef = node, attributes: attributes2 = null, declaration = node, expression: expression2 = node, key = node, markup = node, options: options2 = null, value: value2 = node, variant: variant2 = node } = visitors;
  const handleOptions = (options_, context) => {
    if (options_) {
      const end = options2?.(options_, context);
      if (value2) {
        for (const value_ of Object.values(options_)) {
          value2(value_, context, "option");
        }
      }
      end?.();
    }
  ...
  const handleAttributes = (attributes_, context) => {
    if (attributes_) {
      const end = attributes2?.(attributes_, context);
      if (value2) {
        for (const value_ of Object.values(attributes_)) {
          if (value_ !== true)
            value2(value_, context, "attribute");
        }
      }
      end?.();
    }
  ...
  const handleElement = (exp, context) => {
    if (typeof exp === "object") {
      ...
      switch (exp.type) {
        case "expression": {
          end = expression2?.(exp, context);
          if (exp.arg)
            value2?.(exp.arg, context, "arg");
          if (exp.functionRef) {
  ...
  if (msg.type === "message") {
    ...
    if (value2)
      for (const sel of msg.selectors)
        value2(sel, "selector", "arg");
    for (const vari of msg.variants) {
      const end = variant2?.(vari);
      if (key)
...
// ../../node_modules/messageformat/lib/data-model/validate.js
function validate(msg, onError = (type, node) => {
  ...
  visit(msg, {
    declaration(decl) {
      if (!decl.name)
        return void 0;
      if (decl.value.functionRef || decl.type === "local" && decl.value.arg?.type === "variable" && annotated.has(decl.value.arg.name)) {
    ...
    value(value2, context, position) {
      if (value2.type !== "variable")
        return;
      variables.add(value2.name);
      switch (context) {
        case "declaration":
          if (position !== "arg" || setArgAsDeclared) {
            declared.add(value2.name);
          }
          break;
        case "selector":
          selectorCount += 1;
          missingFallback = value2;
          if (!annotated.has(value2.name)) {
            onError("missing-selector-annotation", value2);
          }
      }
    },
    variant(variant2) {
      ...
      if (keys.length !== selectorCount)
        onError("key-mismatch", variant2);
      const strKeys = JSON.stringify(keys.map((key) => key.type === "literal" ? key.value : 0));
      if (variants.has(strKeys))
        onError("duplicate-variant", variant2);
      else
        variants.add(strKeys);
      missingFallback &&= keys.every((key) => key.type === "*") ? null : variant2;
    }
  });
  if (missingFallback)
...
// ../../node_modules/messageformat/lib/dir-utils.js
var LRI = "⁦";
var RLI = "⁧";
var FSI = "⁨";
var PDI = "⁩";
var RTL = "Adlm,Arab,Hebr,Mand,Nkoo,Rohg,Syrc,Thaa";
...
// ../../node_modules/messageformat/lib/functions/utils.js
...
function asPositiveInteger(value2) {
  if (value2 && typeof value2 === "object")
    value2 = value2.valueOf();
  if (value2 && typeof value2 === "object")
    value2 = String(value2);
  if (typeof value2 === "string" && /^(0|[1-9][0-9]*)$/.test(value2)) {
    value2 = Number(value2);
  }
  if (typeof value2 === "number" && value2 >= 0 && Number.isInteger(value2)) {
    return value2;
  }
...
// ../../node_modules/messageformat/lib/functions/number.js
function readNumericOperand(value2) {
  let options2 = void 0;
  if (typeof value2 === "object") {
    const valueOf = value2?.valueOf;
    if (typeof valueOf === "function") {
      options2 = value2.options;
      value2 = valueOf.call(value2);
    }
  }
  if (typeof value2 === "string") {
    try {
  ...
  return { value: value2, options: options2 };
}
function getMessageNumber(ctx, value2, options2, canSelect) {
  let { dir, locales } = ctx;
  if (options2.useGrouping === "never")
    options2.useGrouping = false;
  if (canSelect && "select" in options2 && !ctx.literalOptionKeys.has("select")) {
  ...
  return {
    ...
    get dir() {
      if (dir == null) {
        locale ??= Intl.NumberFormat.supportedLocalesOf(locales, options2)[0];
        dir = getLocaleDir(locale);
      }
      return dir;
    },
    get options() {
      return __spreadValues({}, options2);
    },
    selectKey: canSelect ? (keys) => {
      let numVal = value2;
      if (options2.style === "percent") {
        if (typeof numVal === "bigint")
          numVal *= 100n;
        else
          numVal *= 100;
      }
      const str2 = String(numVal);
      if (keys.has(str2))
        return str2;
      if (options2.select === "exact")
        return null;
      const pluralOpt = options2.select ? __spreadProps(__spreadValues({}, options2), { select: void 0, type: options2.select }) : options2;
      cat ??= new Intl.PluralRules(locales, pluralOpt).select(Number(numVal));
      return keys.has(cat) ? cat : null;
    } : void 0,
    toParts() {
      nf ??= new Intl.NumberFormat(locales, options2);
      const parts = nf.formatToParts(value2);
      locale ??= nf.resolvedOptions().locale;
      dir ??= getLocaleDir(locale);
      return dir === "ltr" || dir === "rtl" ? [{ type: "number", dir, locale, parts }] : [{ type: "number", locale, parts }];
    },
    toString() {
      nf ??= new Intl.NumberFormat(locales, options2);
      str ??= nf.format(value2);
      return str;
    },
    valueOf: () => value2
  };
...
function number(ctx, exprOpt, operand) {
  ...
  const options2 = Object.assign({}, input.options, {
    localeMatcher: ctx.localeMatcher,
    style: "decimal"
  });
  for (const [name2, optval] of Object.entries(exprOpt)) {
    if (optval === void 0)
      continue;
    try {
      switch (name2) {
        ...
        case "roundingIncrement":
          options2[name2] = asPositiveInteger(optval);
          break;
        case "roundingMode":
        case "roundingPriority":
        case "select":
        // Called 'type' in Intl.PluralRules
        case "signDisplay":
        case "trailingZeroDisplay":
        case "useGrouping":
          options2[name2] = asString(optval);
      }
    } catch {
      ctx.onError("bad-option", `Value ${optval} is not valid for :number option ${name2}`);
    }
  }
  return getMessageNumber(ctx, value2, options2, true);
}
function integer(ctx, exprOpt, operand) {
  const input = readNumericOperand(operand);
  const value2 = Number.isFinite(input.value) ? Math.round(input.value) : input.value;
  const options2 = Object.assign({}, input.options, {
    //localeMatcher: ctx.localeMatcher,
    maximumFractionDigits: 0,
    minimumFractionDigits: void 0,
    minimumSignificantDigits: void 0,
    style: "decimal"
  });
  for (const [name2, optval] of Object.entries(exprOpt)) {
    if (optval === void 0)
    ...
    try {
      ...
      ctx.onError("bad-option", `Value ${optval} is not valid for :integer option ${name2}`);
    }
  }
  return getMessageNumber(ctx, value2, options2, true);
}

// ../../node_modules/messageformat/lib/functions/currency.js
    ...
    try {
      switch (name2) {
        ...
        case "fractionDigits": {
          const strval = asString(optval);
          if (strval === "auto") {
            options2.minimumFractionDigits = void 0;
            options2.maximumFractionDigits = void 0;
          } else {
            const numval = asPositiveInteger(strval);
            options2.minimumFractionDigits = numval;
            options2.maximumFractionDigits = numval;
          }
          break;
        }
      }
    } catch {
      ctx.onError("bad-option", `Value ${optval} is not valid for :currency option ${name2}`);
    }
  }
  if (!options2.currency) {
    throw new MessageFunctionError("bad-operand", "A currency code is required for :currency");
  }
  return getMessageNumber(ctx, input.value, options2, false);
}

// ../../node_modules/messageformat/lib/functions/datetime.js
var dateFieldsValues = /* @__PURE__ */ new Set([
  "weekday",
  "day-weekday",
  "month-day",
  "month-day-weekday",
...
var dateLengthValues = /* @__PURE__ */ new Set(["long", "medium", "short"]);
var timePrecisionValues = /* @__PURE__ */ new Set(["hour", "minute", "second"]);
var timeZoneStyleValues = /* @__PURE__ */ new Set(["long", "short"]);
var datetime = (ctx, options2, operand) => dateTimeImplementation("datetime", ctx, options2, operand);
var date = (ctx, options2, operand) => dateTimeImplementation("date", ctx, options2, operand);
var time = (ctx, options2, operand) => dateTimeImplementation("time", ctx, options2, operand);
function dateTimeImplementation(functionName, ctx, exprOpt, operand) {
  ...
  if (typeof value2 === "object" && value2 !== null) {
    const opt = value2.options;
    if (opt) {
      options2.calendar = opt.calendar;
      if (functionName !== "date")
        options2.hour12 = opt.hour12;
      options2.timeZone = opt.timeZone;
    }
    if (typeof value2.valueOf === "function")
      value2 = value2.valueOf();
  }
  ...
  if (exprOpt.calendar !== void 0) {
    try {
      options2.calendar = asString(exprOpt.calendar);
    } catch {
      ctx.onError("bad-option", `Invalid :${functionName} calendar option value`);
    }
  }
  if (exprOpt.hour12 !== void 0 && functionName !== "date") {
    try {
      options2.hour12 = asBoolean(exprOpt.hour12);
    } catch {
      ctx.onError("bad-option", `Invalid :${functionName} hour12 option value`);
    }
  }
  if (exprOpt.timeZone !== void 0) {
    let tz;
    try {
  ...
  if (functionName !== "time") {
    ...
    if (dateFields.has("month")) {
      options2.month = dateLength === "long" ? "long" : dateLength === "short" ? "numeric" : "short";
    }
    if (dateFields.has("day"))
      options2.day = "numeric";
    if (dateFields.has("weekday")) {
      options2.weekday = dateLength === "long" ? "long" : "short";
    }
  }
  if (functionName !== "date") {
    ...
    switch (readStringOption(ctx, exprOpt, tpName, timePrecisionValues)) {
      ...
      case "second":
        ...
        options2.second = "numeric";
        break;
      default:
        options2.hour = "numeric";
        options2.minute = "numeric";
    }
    options2.timeZoneName = readStringOption(ctx, exprOpt, "timeZoneStyle", timeZoneStyleValues);
  }
  const dtf = new Intl.DateTimeFormat(ctx.locales, options2);
  let dir = ctx.dir;
  let locale;
  let str;
  return {
    ...
    toParts() {
      const parts = dtf.formatToParts(value2);
      locale ??= dtf.resolvedOptions().locale;
      dir ??= getLocaleDir(locale);
      return dir === "ltr" || dir === "rtl" ? [{ type: "datetime", dir, locale, parts }] : [{ type: "datetime", locale, parts }];
    },
    toString() {
      str ??= dtf.format(value2);
      return str;
    },
    valueOf: () => value2
  };
}
function readStringOption(ctx, options2, name2, allowed) {
  const value2 = options2[name2];
  if (value2 !== void 0) {
    try {
      const str = asString(value2);
      if (allowed && !allowed.has(str))
        throw Error();
      return str;
    } catch {
      ctx.onError("bad-option", `Invalid value for ${name2} option`);
    }
  }
  return void 0;
}

// ../../node_modules/messageformat/lib/functions/offset.js
function offset(ctx, exprOpt, operand) {
  let { value: value2, options: options2 } = readNumericOperand(operand);
  let add;
  try {
  ...
  if (typeof value2 === "number")
    ...
    value2 += BigInt(delta);
  return number(ctx, {}, { valueOf: () => value2, options: options2 });
}

// ../../node_modules/messageformat/lib/functions/percent.js
function percent(ctx, exprOpt, operand) {
  const input = readNumericOperand(operand);
  const options2 = Object.assign({}, input.options, {
    localeMatcher: ctx.localeMatcher,
    style: "percent"
  ...
  for (const [name2, optval] of Object.entries(exprOpt)) {
    ...
    try {
      ...
      ctx.onError("bad-option", `Value ${optval} is not valid for :percent option ${name2}`);
    }
  }
  return getMessageNumber(ctx, input.value, options2, true);
}

// ../../node_modules/messageformat/lib/functions/string.js
function string(ctx, _options, operand) {
  ...
  return {
    ...
    toParts() {
      ...
      return dir === "ltr" || dir === "rtl" ? [{ type: "string", dir, locale, value: str }] : [{ type: "string", locale, value: str }];
    },
    toString: () => str,
    valueOf: () => str
  };
...
// ../../node_modules/messageformat/lib/functions/unit.js
...
// ../../node_modules/messageformat/lib/functions/index.js
...
DefaultFunctions = Object.freeze(Object.assign(/* @__PURE__ */ Object.create(null), DefaultFunctions));
var DraftFunctions = {
  /**
...
DraftFunctions = Object.freeze(Object.assign(/* @__PURE__ */ Object.create(null), DraftFunctions));

// ../../node_modules/messageformat/lib/message-value.js
var BIDI_ISOLATE = Symbol("bidi-isolate");

// ../../node_modules/messageformat/lib/functions/fallback.js
var fallback = (source2 = "�") => ({
  type: "fallback",
  source: source2,
  toParts: () => [{ type: "fallback", source: source2 }],
  toString: () => `{${source2}}`
});

// ../../node_modules/messageformat/lib/functions/unknown.js
var unknown = (source2, input) => ({
  type: "unknown",
  source: source2,
  dir: "auto",
...
// ../../node_modules/messageformat/lib/resolve/function-context.js
var MessageFunctionContext = class {
  #ctx;
  #litKeys;
  #source;
  dir;
  id;
  constructor(ctx, source2, options2) {
    this.#ctx = ctx;
    this.#source = source2;
    this.dir = void 0;
    const dirOpt = options2 && Object.hasOwn(options2, "u:dir") ? options2["u:dir"] : void 0;
    if (dirOpt) {
    ...
    const idOpt = options2 && Object.hasOwn(options2, "u:id") ? options2["u:id"] : void 0;
    this.id = idOpt ? String(resolveValue(ctx, idOpt)) : void 0;
    if (options2) {
      this.#litKeys = /* @__PURE__ */ new Set();
      for (const [key, value2] of Object.entries(options2)) {
        if (value2.type === "literal")
...
// ../../node_modules/messageformat/lib/resolve/resolve-function-ref.js
function resolveFunctionRef(ctx, operand, { name: name2, options: options2 }) {
  const fnSource = `:${name2}`;
  const source2 = getValueSource(operand) ?? fnSource;
  try {
    const fnInput = operand ? [resolveValue(ctx, operand)] : [];
    const rf = ctx.functions[name2];
    if (!rf) {
      throw new MessageResolutionError("unknown-function", `Unknown function ${fnSource}`, source2);
    }
    const msgCtx = new MessageFunctionContext(ctx, source2, options2);
    const opt = resolveOptions(ctx, options2);
    const res = rf(msgCtx, opt, ...fnInput);
    if (res === null || typeof res !== "object" || typeof res.type !== "string") {
      throw new MessageResolutionError("bad-function-result", `Function ${fnSource} did not return a MessageValue`, source2);
    }
    const override = { source: source2 };
    if (msgCtx.dir) {
    ...
    return __spreadValues(__spreadValues({}, res), override);
  } catch (error) {
    ctx.onError(error instanceof MessageError ? error : new MessageResolutionError("bad-function-result", String(error), source2, error));
    return fallback(source2);
  }
}
function resolveOptions(ctx, options2) {
  const opt = /* @__PURE__ */ Object.create(null);
  if (options2) {
    for (const [name2, value2] of Object.entries(options2)) {
      if (!name2.startsWith("u:"))
        opt[name2] = resolveValue(ctx, value2);
    }
  }
  return opt;
}

// ../../node_modules/messageformat/lib/resolve/resolve-expression.js
function resolveExpression(ctx, { arg, functionRef }) {
  ...
  switch (arg?.type) {
    case "literal": {
      const source2 = `|${arg.value}|`;
      const msgCtx = new MessageFunctionContext(ctx, source2);
      const msgStr = string(msgCtx, {}, arg.value);
      msgStr.source = source2;
      return msgStr;
    }
    case "variable":
...
// ../../node_modules/messageformat/lib/resolve/resolve-variable.js
var UnresolvedExpression = class {
  expression;
  scope;
  constructor(expression2, scope) {
    this.expression = expression2;
    this.scope = scope;
  }
};
var isScope = (scope) => scope !== null && (typeof scope === "object" || typeof scope === "function");
function getValue(scope, name2) {
  if (isScope(scope)) {
    if (name2 in scope)
      return scope[name2];
    ...
    for (const [key, value2] of Object.entries(scope)) {
      if (key.normalize() === name2)
        return value2;
    }
  }
  return void 0;
}
function lookupVariableRef(ctx, { name: name2 }) {
  const value2 = getValue(ctx.scope, name2);
  if (value2 === void 0) {
    const source2 = "$" + name2;
    const msg = `Variable not available: ${source2}`;
    ctx.onError(new MessageResolutionError("unresolved-variable", msg, source2));
  } else if (value2 instanceof UnresolvedExpression) {
    const local = resolveExpression(value2.scope ? __spreadProps(__spreadValues({}, ctx), { scope: value2.scope }) : ctx, value2.expression);
    ctx.scope[name2] = local;
...
function resolveVariableRef(ctx, ref) {
  const source2 = "$" + ref.name;
  const value2 = lookupVariableRef(ctx, ref);
  if (value2 === void 0)
    return fallback(source2);
  let type = typeof value2;
  if (type === "object") {
    const mv2 = value2;
    if (mv2.type === "fallback")
      return fallback(source2);
    if (ctx.localVars.has(mv2)) {
      mv2.source = source2;
      return mv2;
    }
  ...
  switch (type) {
    ...
    case "string":
      ...
      break;
    default:
      return unknown(source2, value2);
  }
  const msgCtx = new MessageFunctionContext(ctx, source2);
  const mv = msgFn(msgCtx, {}, value2);
  mv.source = source2;
  return mv;
}

// ../../node_modules/messageformat/lib/resolve/resolve-value.js
function resolveValue(ctx, value2) {
  switch (value2.type) {
    case "literal":
      return value2.value;
    case "variable":
      return lookupVariableRef(ctx, value2);
    default:
      throw new Error(`Unsupported value: ${value2.type}`);
  }
}
function getValueSource(value2) {
  switch (value2?.type) {
    case "literal":
      return "|" + value2.value.replaceAll("\\", "\\\\").replaceAll("|", "\\|") + "|";
    case "variable":
      return "$" + value2.name;
    default:
      return void 0;
...
// ../../node_modules/messageformat/lib/resolve/format-markup.js
function formatMarkup(ctx, { kind, name: name2, options: options2 }) {
  const part = { type: "markup", kind, name: name2 };
  const entries = options2 ? Object.entries(options2) : null;
  if (entries?.length) {
    part.options = {};
    for (const [name3, value2] of entries) {
      if (name3 === "u:dir") {
        const error = new MessageFunctionError("bad-option", `The option ${name3} is not valid for markup`);
        error.source = getValueSource(value2);
        ctx.onError(error);
      } else {
        let rv = resolveValue(ctx, value2);
...
// ../../node_modules/messageformat/lib/select-pattern.js
...
var MessageFormat = class {
  ...
  #message;
  #functions;
  constructor(locales, source2, options2) {
    this.#bidiIsolation = options2?.bidiIsolation !== "none";
    this.#localeMatcher = options2?.localeMatcher ?? "best fit";
    this.#locales = Array.isArray(locales) ? locales.map((lc) => new Intl.Locale(lc)) : locales ? [new Intl.Locale(locales)] : [];
    this.#dir = options2?.dir ?? getLocaleDir(this.#locales[0]);
    this.#message = typeof source2 === "string" ? parseMessage(source2) : source2;
    validate(this.#message);
    this.#functions = options2?.functions ? Object.assign(/* @__PURE__ */ Object.create(null), DefaultFunctions, options2.functions) : DefaultFunctions;
  }
  /**
      ...
      if (typeof elem === "string") {
        ...
        try {
          ...
          if (typeof mv.toString === "function") {
            if (this.#bidiIsolation && (this.#dir !== "ltr" || mv.dir !== "ltr" || mv[BIDI_ISOLATE])) {
  ...
  #createContext(msgParams, onError = (error) => {
    ...
    const scope = __spreadValues({}, msgParams);
    for (const decl of this.#message.declarations) {
      scope[decl.name] = new UnresolvedExpression(decl.value, decl.type === "input" ? msgParams ?? {} : void 0);
    }
    const ctx = {
      onError,
      localeMatcher: this.#localeMatcher,
      locales: this.#locales,
      localVars: /* @__PURE__ */ new WeakSet(),
...
// ../../dist/angular-mf2/fesm2022/angular-mf2.mjs
var import_sanitize_html = __toESM(require_sanitize_html(), 1);
var AngularMf2 = class _AngularMf2 {
  static ɵfac = function AngularMf2_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _AngularMf2)();
  };
  static ɵcmp = ɵɵdefineComponent({ type: _AngularMf2, selectors: [["lib-angular-mf2"]], decls: 2, vars: 0, template: function AngularMf2_Template(rf, ctx) {
    if (rf & 1) {
      ɵɵdomElementStart(0, "p");
      ɵɵtext(1, " angular-mf2 works! ");
      ɵɵdomElementEnd();
    }
  }, encapsulation: 2 });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(AngularMf2, [{
    type: Component,
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && ɵsetClassDebugInfo(AngularMf2, { className: "AngularMf2", filePath: "lib/angular-mf2.ts", lineNumber: 13 });
})();
var i18nConfig = {
  defaultLocale: "en",
  catalogs: {
...
function mf2Format(locale, t, a) {
  ...
  for (const key in t) {
    ...
    result[key] = (0, import_sanitize_html.default)(formatted, {
      allowedTags: ["b", "i", "a", "span"],
      // Only allow tags used by mf2
      allowedAttributes: {},
...
function markupToHtml(part) {
  const name2 = part.name?.trim().toLowerCase();
  switch (name2) {
    case "bold":
...
var Store = class _Store {
  ...
  static ɵprov = ɵɵdefineInjectable({ token: _Store, factory: _Store.ɵfac, providedIn: "root" });
};
(() => {
...
var I18nPipe = class _I18nPipe {
  ...
  static ɵfac = function I18nPipe_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _I18nPipe)(ɵɵdirectiveInject(Store, 16));
  };
  static ɵpipe = ɵɵdefinePipe({ name: "i18n", type: _I18nPipe, pure: false });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(I18nPipe, [{
    ...
    args: [{
      ...
      standalone: true,
      pure: false
      // <— impure so it re-evaluates when Store changes
    }]
  }], () => [{ type: Store }], null);
})();
export {
